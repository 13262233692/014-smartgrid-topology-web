import G6, { Graph, NodeConfig, EdgeConfig, LayoutConfig } from '@antv/g6';
import { EquipmentNode, ConnectionEdge, EquipmentType } from '../types';
...
function convertToG6Data(
  ...
  const g6Nodes: NodeConfig[] = nodes.map((node) => {
    ...
    const color = !isEnergized ? '#64748b' : baseColor;
    const strokeColor = inCycle ? '#ef4444' : color;
    const lineWidth = inCycle ? 4 : 2;
    ...
    else if (node.type === EquipmentType.ENERGY_CONSUMER) size = 24;
    ...
    const shape =
      node.type === EquipmentType.BUSBAR_SECTION
        ...
        ? 'breaker-node'
        : node.type === EquipmentType.SUBSTATION
        ? 'circle'
        : 'circle';
    ...
    return {
      ...
      labelCfg: {
        style: {
          fill: inCycle ? '#fca5a5' : '#e2e8f0',
          fontSize: 11,
          fontWeight: inCycle ? 700 : 500,
        },
...
export function updateGraphData(
  ...
  const { nodes: g6Nodes, edges: g6Edges } = convertToG6Data(nodes, edges);
