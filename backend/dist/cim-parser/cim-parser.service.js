...
exports.CimParserService = void 0;
const common_1 = require("@nestjs/common");
const fast_xml_parser_1 = require("fast-xml-parser");
const cim_types_1 = require("./cim.types");
let CimParserService = class CimParserService {
    constructor() {
        this.RDF_PREFIX = 'cim:';
        this.ID_ATTR = 'rdf:ID';
        this.ABOUT_ATTR = 'rdf:about';
        this.RESOURCE_ATTR = 'rdf:resource';
    }
    ...
    extractTopology(jsonObj) {
        const nodes = [];
        const edges = [];
        const terminalMap = new Map();
        const rdfRoot = jsonObj['rdf:RDF'] || jsonObj['RDF'] || jsonObj;
        const typeMappings = [
            { type: cim_types_1.EquipmentType.SUBSTATION, tags: ['cim:Substation', 'Substation'] },
            { type: cim_types_1.EquipmentType.BUSBAR_SECTION, tags: ['cim:BusbarSection', 'BusbarSection'] },
            { type: cim_types_1.EquipmentType.BREAKER, tags: ['cim:Breaker', 'Breaker', 'cim:Disconnector', 'Disconnector'] },
            { type: cim_types_1.EquipmentType.POWER_TRANSFORMER, tags: ['cim:PowerTransformer', 'PowerTransformer'] },
            { type: cim_types_1.EquipmentType.TRANSFORMER_WINDING, tags: ['cim:TransformerWinding', 'TransformerWinding'] },
            { type: cim_types_1.EquipmentType.FEEDER, tags: ['cim:Feeder', 'Feeder', 'cim:Line', 'Line', 'cim:ACLineSegment', 'ACLineSegment'] },
            { type: cim_types_1.EquipmentType.CONNECTIVITY_NODE, tags: ['cim:ConnectivityNode', 'ConnectivityNode'] },
            { type: cim_types_1.EquipmentType.TERMINAL, tags: ['cim:Terminal', 'Terminal'] },
            { type: cim_types_1.EquipmentType.ENERGY_CONSUMER, tags: ['cim:EnergyConsumer', 'EnergyConsumer'] },
            { type: cim_types_1.EquipmentType.ENERGY_SOURCE, tags: ['cim:EnergySource', 'EnergySource'] },
        ];
        for (const mapping of typeMappings) {
            for (const tag of mapping.tags) {
                ...
                for (const el of elements) {
                    ...
                    if (node) {
                        nodes.push(node);
                        if (mapping.type === cim_types_1.EquipmentType.TERMINAL && node.connectivityNodeId) {
    ...
    buildConnections(nodes, terminalMap) {
        ...
        const pushEdge = (sourceId, targetId, relationship) => {
        ...
        for (const node of nodes) {
            switch (node.type) {
                case cim_types_1.EquipmentType.TRANSFORMER_WINDING:
                    if (node.transformerId) {
                        pushEdge(node.transformerId, node.id, 'HAS_WINDING');
                    }
                    break;
                case cim_types_1.EquipmentType.BUSBAR_SECTION:
                case cim_types_1.EquipmentType.BREAKER:
                case cim_types_1.EquipmentType.ENERGY_CONSUMER:
                case cim_types_1.EquipmentType.ENERGY_SOURCE:
        ...
        const byCN = new Map();
        for (const node of nodes) {
            if (node.type !== cim_types_1.EquipmentType.TERMINAL && node.connectivityNodeId) {
        ...
        const cnNodes = nodes.filter(n => n.type === cim_types_1.EquipmentType.CONNECTIVITY_NODE);
    ...
    generateDemoData() {
        ...
        for (const ss of substations) {
            nodes.push({
                ...
                mrid: ss.id,
                name: ss.name,
                type: cim_types_1.EquipmentType.SUBSTATION,
        ...
        for (const cfg of busbarConfig) {
            for (let i = 1; i <= cfg.count; i++) {
                ...
                nodes.push({
                    ...
                    type: cim_types_1.EquipmentType.BUSBAR_SECTION,
        ...
        for (const [tid, ssId, busId, hv, lv] of transformerPairs) {
            nodes.push({
                ...
                mrid: tid,
                name: `主变${tid.slice(-1)}`,
                type: cim_types_1.EquipmentType.POWER_TRANSFORMER,
                substationId: ssId,
                powerRating: 50,
                energized: true,
            });
            ...
            nodes.push({
                ...
                mrid: w1Id,
                name: `高压绕组`,
                type: cim_types_1.EquipmentType.TRANSFORMER_WINDING,
                transformerId: tid,
                ratedKV: hv,
                sequenceNumber: 1,
                energized: true,
        ...
        for (const cfg of breakerConfig) {
            for (let i = 1; i <= cfg.count; i++) {
                ...
                nodes.push({
                    ...
                    type: cim_types_1.EquipmentType.BREAKER,
                    baseVoltage: cfg.voltage,
                    substationId: cfg.ss,
                    normalOpen: isOpen,
                    open: isOpen,
        ...
        for (const fd of feederData) {
            nodes.push({
                ...
                mrid: fd.id,
                name: fd.name,
                type: cim_types_1.EquipmentType.FEEDER,
        ...
        nodes.push({
            ...
            mrid: sourceId,
            name: '省级电网',
            type: cim_types_1.EquipmentType.ENERGY_SOURCE,
            baseVoltage: 500,
            energized: true,
        });
...
exports.CimParserService = CimParserService;
exports.CimParserService = CimParserService = __decorate([
    (0, common_1.Injectable)()
], CimParserService);
//# sourceMappingURL=cim-parser.service.js.map